...
uint sark_msg_send (sdp_msg_t *msg, uint timeout)
{
  ...
  io_printf (IO_BUF, "sv->mbox_flags %x, sc[SC_SET_IRQ] %x\n",sv->mbox_flags,t,sc[SC_SET_IRQ] );
  ...
  //io_printf (IO_BUF, "Enter:sark_msg_send\n");

  //io_printf (IO_BUF, "INIT: Allocate NUM[%d] SDP packet\n",sark_vec->num_msgs);  
  
  //io_printf (IO_BUF, "sark_msg_send:sark.vcpu->mbox_mp_cmd %d\n", sark.vcpu->mbox_mp_cmd);  
  ...
  if (sark.vcpu->mbox_mp_cmd != SHM_IDLE)
    {
	  ...
	  //io_printf (IO_BUF, "Leave:NOT Normal:sark_msg_send\n");
  ...
  //io_printf (IO_BUF, "Sark:send sdp message counter %d\n",sark.msg_sent);

  //io_printf (IO_BUF, "Leave:Normal:sark_msg_send\n");
...
void sark_int (void *pc)
{
  ...
  sdp_msg_t *shm_msg_temp = (sdp_msg_t *) data;
  
  io_printf (IO_BUF, "(1)------- sark_int: cmd %d,data %d\n",cmd,*((uint *) shm_msg_temp->data));
  ...
  if (cmd == SHM_MSG)
    {
	  ...
	  io_printf (IO_BUF, "(1)------- sark_msg_get\n");
      ...
      if (msg != NULL)
	{
	  ...
	  io_printf (IO_BUF, "(2)------- sark_msg_get\n");
	  ...
	  if ((dp & PORT_MASK) == 0) // Port 0 is for us
		...
		{
#ifdef SARK_API		  
		  io_printf (IO_BUF, "(3)------- sark_msg_get:%d\n",*((uint *) msg->data));
 ...
 if (cmd == SHM_MSG)
      ...
      if (msg != NULL)
	{
	  ...
	  if ((dp & PORT_MASK) == 0) // Port 0 is for us
		...
		{
#ifdef SARK_API		  
		  io_printf (IO_BUF, "(3)------- sark_msg_get\n");
		  if (callback[SDP_PACKET_RX].cback != NULL)
		    {
		      uint cpsr = cpu_int_disable ();
