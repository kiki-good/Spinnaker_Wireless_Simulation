...
void c_main (void)
{
  ...
  sark_srand ((sark_chip_id () << 5) + core * sv->time_ms); // Init randgen

  /* JM_DEBUG */
  //timer_register (SLOT_0);		// Set up the timer event mechanism
  event_register_timer(SLOT_0);
  
  event_queue_proc (timer_proc, 0, 0, PRIO_0); // Queue the first timer call

  uint rc = event_start (0, 0, 0);		// Start event handling
